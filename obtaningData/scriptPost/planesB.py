...
def di_calc(ncli, Li, ni):
    if ncli == 0:  ## ncl = 0 eh periodico
        return float(Li) / (float(ni) - 0)
    else:
        return float(Li) / (float(ni) - 1)
#                   condicao de contorno,     compr. dominio, nro de pontos
...
x = np.linspace(0, simulation['Lx'], simulation['nx'], endpoint = True) #np.arange(0, simulation['Lx'] + simulation['dx']/2, simulation['dx'])
y = np.linspace(0, simulation['Ly'], simulation['ny'], endpoint = True) #np.arange(0, simulation['Lx'] + simulation['dx']/2, simulation['dx'])
z = np.linspace(0, simulation['Lz'], simulation['nz'], endpoint = True) #np.arange(0, simulation['Lx'] + simulation['dx']/2, simulation['dx'])
...
def plot_plane_xy_cut (field):
    for i in range(simulation['ifirst'], simulation['ifinal']):  # UX
        ...
        A = A.reshape((simulation['nz'], simulation['ny'], simulation['nx']), order='FORTRAN')
        ...
        plt.ylabel(r'$y$')
        plt.xlabel(r'$x$')
        plt.contourf(X, Y, A[0,:,:], 256, cmap=cm, vmin=0., vmax=1.)
        plt.colorbar()
        plt.grid(True)
        plt.minorticks_on()        
...
def plot_plane_zy_cut (field, ixCut, xCut):
    #Levels = np.linspace(0., 1., 256, endpoint = True)
    Levels = np.array([0.1])
    for i in range(simulation['ifirst'], simulation['ifinal']-simulation['ifirst']):  # UX
        #xCut = xFront[i] - tol
        filename = simulation['prefix'] + '/' + simulation['name'] + simulation['dataplace'] + field + str(i).zfill(4)
        print(filename)
        A = np.fromfile(filename, dtype=np.float32)
        ...
        A = A.reshape((simulation['nz'], simulation['ny'], simulation['nx']))#, order='FORTRAN')     
        #A = A.reshape((simulation['nx'], simulation['ny']), order='FORTRAN')     
        plt.figure(1, figsize=(simulation['Lz'], simulation['Ly'] * 1.5))
        plt.axes().set_aspect('equal')
        plt.ylabel(r'$y$')
        plt.xlabel(r'$z$')
        plt.title(r'$x_{cut} = %2.4f$' %xCut)
        #plt.contourf(zz, yy, A[:,:,ixCut], cmap=cm, levels = Levels)
        cs = plt.contour(zz, yy, A[:,:,ixCut], colors='k', levels = Levels) ## k = black, b = blue
        ## Isso eh para colocar as linhas em arrays
        #cs.collections[:].get_paths() #https://stackoverflow.com/questions/5666056/matplotlib-extracting-data-from-contour-lines
        #plt.colorbar()
        plt.grid(True)
        plt.minorticks_on()        
        #plt.savefig(simulation['output'] +   '/plano_' + field + str(i).zfill(4) + '.svg', format='svg', dpi=300)  # 'pdf')
        plt.savefig(simulation['output'] +   '/plano_xCut_' + '%2.2f'  %xCut+ '_' + field + str(i).zfill(4) + '.png', format='png', dpi=300)  # 'pdf')
...
#plot_plane_xy_cut('phi1')
xCutMin = 0.
xCutMax = simulation['Lx']/2.
nCut = 4
xCut = np.linspace(xCutMin, xCutMax, nCut, endpoint = True) ## Isso eh um arranjo
# divide por 2 pra ir ateh a metade do dominio

for ii in range(0, nCut):
    ixCut = int(xCut[ii]/simulation['dx'])
    plot_plane_zy_cut('phi1', ixCut, xCut[ii])
